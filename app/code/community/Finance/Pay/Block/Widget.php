...
class Finance_Pay_Block_Widget extends Mage_Core_Block_Template
{
    ...
    public function isActive ($price = null)
    {
        if ($this->active === null) {
            ...
            $active  = Mage::helper('finance_pay')->isActiveLocal($product, $price);
    ...
    public function getPlans ()
    {
        if ($this->plans === null) {
            $product = $this->getProduct();
            $plans   = Mage::helper('finance_pay')->getLocalPlans($product);
            $plans   = Mage::helper('finance_pay')->plans2list($plans);
