...
class FinanceProvicer_Pay_PaymentController extends Mage_Core_Controller_Front_Action
{
    ...
    public function startAction()
    {
        ...
        $salt = uniqid('', true);
        $quote_hash = Mage::helper('finance_provider_pay')->hashQuote($salt, $quote_id);
        ...
        if (Mage::getStoreConfig('payment/pay/debug')) {
            Mage::log('Request: ' . json_encode($request_data), Zend_Log::DEBUG, 'finance_provider.log', true);
        }

        $response = Divido_CreditRequest::create($request_data);

        if (Mage::getStoreConfig('payment/pay/debug')) {
            Mage::log('Response: ' . $response->__toJSON(), Zend_Log::DEBUG, 'finance_provider.log', true);
        }
        ...
        if ($response->status == 'ok') {
            ...
            if (Mage::getStoreConfig('payment/pay/debug')) {
                Mage::log('Lookup: ' . json_encode($lookup->getData()), Zend_Log::DEBUG, 'finance_provider.log', true);
            }
    ...
    public function returnAction ()
    {
        ...
        if (Mage::getStoreConfig('payment/pay/debug')) {
            Mage::log('Return, quote ID: ' . $quoteId, Zend_Log::DEBUG, 'finance_provider.log', true);
        }
        ...
        while($i < self::WAIT_TIME) {
            ...
            if ($order->getId()) {
                if (Mage::getStoreConfig('payment/pay/debug')) {                    
                    Mage::log('already have order with id ' . $quoteId, Zend_Log::DEBUG, 'finance_provider.log', true);
                }
                break;
            }else { 
                if (Mage::getStoreConfig('payment/pay/debug')) {                    
                    Mage::log('order not created waiting: ' . $quoteId, Zend_Log::DEBUG, 'finance_provider.log', true);
                }        
                $i++;
                sleep(1);
            }
    ...
    public function webhookAction ()
    {
        ...
        if (!empty($secretEnc)) {
            ...
            $signature = Mage::helper('finance_provider_pay')->createSignature($payload);
        ...
        $salt = $lookup->getSalt();
        $hash = Mage::helper('finance_provider_pay')->hashQuote($salt, $this->quoteId);
    ...
    private function respond ($ok = true, $message = '', $bad_reqeust = false)
    {
        ...
        $pluginVersion = (string) Mage::getConfig()->getModuleConfig("FinanceProvider_Pay")->version;
